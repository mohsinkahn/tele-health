...
export default function MultiSelectTags({ options = [] }) {
        ...
        <div className="absolute w-full bg-white border border-[#355E5B52] mt-2 rounded-sm shadow-sm z-10 max-h-60 overflow-y-auto">
          {options.map((option) => (
            <div
              ...
              className={`px-4 py-2 text-sm cursor-pointer hover:bg-[#355E5B] hover:text-white ${
                selected.includes(option) ? "bg-[#EEF4F3]" : ""
              }`}
            >
              {option}
            </div>
